...
public class JdbcUserDao implements UserDao {
    ...
    public List<User> getUsersByTournament(int tournamentID) {
        String sql = "SELECT user_id FROM tournaments_users where tournament_id = ?";
        List<Integer> idList = new ArrayList<>();
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, tournamentID);
        while (results.next()) {
            idList.add(results.getInt("user_id"));
        }
        ...
        for(Integer userID:idList){
            ...
            while (results2.next()){
                ...
                User user = new User(num, results2.getString("username"));
