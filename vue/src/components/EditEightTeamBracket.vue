<template>
  <div>
    <div class="buttons">
      <button @click="submitWinners(winners)">Submit Winners</button>
      <button @click="revert()">Cancel Submitting Winners</button>
    </div>
    <main id="tournament">
      ...
      <ul class="round round-4">
        ...
        <li class="game game-top winner">
          <select v-model="winners[6]" name="winner" id="winner">
            <option value="">TBA</option>
            <option v-for="user in users" :value="user.username" :key="user.id">
              {{ user.username }}
            </option>
          </select>
        </li>
...
export default {
  ...
  name: "edit-eight-team-bracket",
  props: {
    tournamentID: Number,
  },
  created() {
    tournamentService
      .getTournamentUsers(this.$store.state.activeTournament.tournamentID)
      .then((userData) => {
        this.users = userData.data;
        while (
          this.users.length < this.$store.state.activeTournament.bracketSize
        ) {
          this.users.push({
    ...
    tournamentService
      .getTournamentWinners(this.$store.state.activeTournament.tournamentID)
      .then((winnerData) => {
        this.winners = winnerData.data;
        while (
          this.winners.length <
          this.$store.state.activeTournament.bracketSize - 1
        ) {
          this.winners.push("");
        }
      });
  ...
  methods: {
    submitWinners(winners) {
      tournamentService.assignWinners(
        this.$store.state.activeTournament.tournamentID,
        winners
      );
      this.$router.push({
        name: "tournament-details",
        params: { id: this.$store.state.activeTournament.tournamentID },
      });
      this.$router.go();
      this.winners = [];
    },
